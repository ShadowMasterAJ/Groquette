...
def parse_meeting_code() -> str:
    """Parse and validate meeting code from arguments or user input."""
    parser = argparse.ArgumentParser(
        description="AI Video Call Assistant for Google Meet"
    )
    parser.add_argument(
        "meeting_code", nargs="?", help="Google Meet code in format: xxx-xxxx-xxx"
    )
    ...
    if (
        len(code_parts) != 3
        or len(code_parts[0]) != 3
        or len(code_parts[1]) != 4
        or len(code_parts[2]) != 3
    ):
...
def run_assistant(meeting_code: str) -> str:
        ...
        while True:
            try:
                ...
                if user_input == "r":
                    ...
                    if user_input:
                        print(
                            "Unknown command. Press 'r' to restart or Ctrl+C to exit."
                        )
...
def restart_application(meeting_code: str) -> None:
